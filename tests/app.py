...
def run_tests():
    ...
    module_funcs = [getattr(module, func) for func in dir(module) if callable(getattr(module, func))]
    ...
    for i, test in enumerate(tests):
        ...
        try:
            del os.environ["CONTAINER_ID"] # This is so that beta9 cli commands is allowed to execute new functions in a worker
            test()
